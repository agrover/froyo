...
impl<'a> Froyo<'a> {
    pub fn new<T>(name: &str, paths: &[T], force: bool) -> FroyoResult<Froyo<'a>>
    where
        T: borrow::Borrow<Path>,
    {
        if paths.len() < MIN_BLK_DEVS {
            return Err(FroyoError::Io(io::Error::new(
                ErrorKind::InvalidInput,
                "At least 2 block devices must be given",
            )));
        }
        ...
        if paths.len() > MAX_BLK_DEVS {
            return Err(FroyoError::Io(io::Error::new(
                ErrorKind::InvalidInput,
                format!(
                    "Max supported devices is {}, {} given",
                    MAX_BLK_DEVS,
                    paths.len()
                ),
            )));
        ...
        let meta_raid_segments = raid_devs.alloc_raid_segments(meta_size).ok_or_else(|| {
            io::Error::new(io::ErrorKind::InvalidInput, "no space for thinpool meta")
        })?;
        let data_raid_segments = raid_devs.alloc_raid_segments(data_size).ok_or_else(|| {
    ...
    pub fn to_metadata_pretty(&self) -> FroyoResult<String> {
        Ok(serde_json::to_string_pretty(&self.to_save())?)
    }
    ...
    pub fn find_all() -> FroyoResult<Vec<Froyo<'a>>> {
        ...
        for (froyo_id, bds) in froyo_devs {
            ...
            let froyo_save = serde_json::from_str::<FroyoSave>(&s)?;
    ...
    fn setup_blockdevs(
        ...
        for bd in bd_map.values() {
            dbgp!(
                "{} header indicates it's part of {} but \
    ...
    pub fn teardown(&mut self) -> FroyoResult<()> {
        ...
        self.thin_pool_dev.teardown(&dm)?;

        self.raid_devs.teardown(&dm)?;
    ...
    pub fn save_state(&self) -> FroyoResult<()> {
        ...
        for bd in self.block_devs.0.values() {
            if let Some(bd) = bd.present() {
                bd.borrow_mut()
                    .save_state(&current_time, metadata.as_bytes())?
            }
        }
    ...
    pub fn status(&self) -> FroyoResult<FroyoState> {
        ...
        if let ThinPoolStatus::Fail = self.thin_pool_dev.status()? {
            return Ok(FroyoState::ThinPoolFailed);
        }

        if let Some(td) = self.thin_devs.get(0) {
            if let ThinStatus::Fail = td.status()? {
        ...
        let cur_running_state = match self.last_state {
            FroyoState::Good(ref running_state) => match running_state {
    ...
    pub fn avail_redundant_space(&self) -> FroyoResult<Sectors> {
        ...
        let thinpool_avail = match self.thin_pool_dev.status()? {
    ...
    pub fn extend_thinpool_data_dev(&mut self, length: Sectors) -> FroyoResult<()> {
        let new_segs = self.raid_devs.alloc_raid_segments(length).ok_or_else(|| {
            io::Error::new(
        ...
        self.thin_pool_dev.extend_data_dev(new_segs)?;
    ...
    pub fn extend_thinpool_meta_dev(&mut self, length: Sectors) -> FroyoResult<()> {
        ...
        self.thin_pool_dev.extend_meta_dev(new_segs)?;
    ...
    pub fn update_dbus(&self) -> FroyoResult<()> {
        if let Some(ref dc) = self.dbus_context {
            let remaining = self.avail_redundant_space()?;
            DbusContext::update_one(&dc.remaining_prop, (*remaining).into())?;
            let total = self.raid_devs.total_space();
            DbusContext::update_one(&dc.total_prop, (*total).into())?;
            ...
            let (status, mut r_status): (u32, u32) = match self.status()? {
            ...
            DbusContext::update_one(&dc.status_prop, status.into())?;
            DbusContext::update_one(&dc.running_status_prop, r_status.into())?;

            let bdev_msg = DbusContext::get_block_devices_msgitem(&self.block_devs);
            DbusContext::update_one(&dc.block_devices_prop, bdev_msg)?;
        }
        Ok(())
    }
                ...
                Ok(found_bd) => {
                    ...
                    if found_bd.froyodev_id == self.id {
                        if self.block_devs.0.contains_key(&found_bd.id) {
                            ...
                            dbgp!(
                                "Block device {} mistakenly believes \
                        ...
                        let buf = found_bd.read_mdax()?;
                        let s = String::from_utf8_lossy(&buf).into_owned();
                        let froyo_save = serde_json::from_str::<FroyoSave>(&s)?;
                        return Err(FroyoError::Froyo(InternalError(
                            format!(
                                "Block device {} is already part of froyodev \
                                     {}, id {}",
                                path.display(),
                                froyo_save.name,
    ...
    pub fn remove_block_device(&mut self, path: &Path, wipe_sb: bool) -> FroyoResult<()> {
        ...
        let mut blockdev = BlockDev::setup(path)?;

        // Check this blockdev is in this froyodev
        if !self.block_devs.0.contains_key(&blockdev.id) {
            return Err(FroyoError::Froyo(InternalError(
                format!("{} is not a member of {}", path.display(), self.name).into(),
            )));
        }
        ...
        let raids_and_linears_using_dev = self
            .raid_devs
            .raids
            .iter_mut()
        ...
        for &(ref raid, _) in &raids_and_linears_using_dev {
            let (status, action) = raid.status()?;
            match status {
                RaidStatus::Good => {}
                RaidStatus::Degraded(x) if x < REDUNDANCY => {}
                _ => {
                    return Err(FroyoError::Froyo(InternalError(
                        format!("Cannot remove {}, a RAID would fail", path.display()).into(),
                    )))
                }
            }
            match action {
                ...
                x => {
                    return Err(FroyoError::Froyo(InternalError(
                        format!(
                            "Cannot remove {}, a RAID is in state {:?}",
                            path.display(),
                            x
                        )
                        .into(),
    ...
    fn check_raidcopy(&mut self, mirror: MirrorDev) -> FroyoResult<ReshapeState> {
        let dm = DM::new()?;

        dbgp!("check_raidcopy");

        if mirror.is_syncing(&dm)? {
        ...
        rld.dev.table_load(&dm, &table)?;
        rld.dev.unsuspend(&dm)?;
    ...
    fn check_copy_to_scratch(&mut self, mirror: MirrorDev) -> FroyoResult<ReshapeState> {
        let dm = DM::new()?;
        ...
        // Can't call save_state below with the rld still borrowed
        {
            ...
            for idx in &mirror.linear_dev_idxs {
                ...
                rld.segments.push(RaidSegment::new(
                    offset,
                    seg_len,
                    RaidLayer::Temp(mirror.dest.clone()),
                ));
    ...
    fn check_copy_from_scratch(&mut self, mirror: MirrorDev) -> FroyoResult<ReshapeState> {
        let dm = DM::new()?;
    ...
    fn reshape_state_machine(&mut self, state: ReshapeState) -> FroyoResult<ReshapeState> {
        match state {
            ...
            ReshapeState::Idle => {
                let r = self.recreate_empty_degraded_raids()?;
                if r.is_busy() {
                ...
                let r = self.start_copy_from_scratch(md)?;
                if r.is_busy() {
                ...
                let r = self.start_copy_from_scratch(dd)?;
                if r.is_busy() {
                    dbgp!("reshaping data from scratch");
                    return Ok(r);
                }

                let r = self.start_copy_to_safe_raid(&self.thin_pool_dev.meta_dev)?;
                if r.is_busy() {
                    dbgp!("reshaping meta to safe raid");
                    return Ok(r);
                }

                let r = self.start_copy_to_safe_raid(&self.thin_pool_dev.data_dev)?;
                if r.is_busy() {
                    dbgp!("reshaping data to safe raid");
                    return Ok(r);
                }

                let r = self.start_copy_to_scratch(&self.thin_pool_dev.meta_dev)?;
                if r.is_busy() {
                    dbgp!("reshaping meta to scratch");
                    return Ok(r);
                }

                let r = self.start_copy_to_scratch(&self.thin_pool_dev.data_dev)?;
                if r.is_busy() {
    ...
    pub fn reshape(&mut self) -> FroyoResult<()> {
        ...
        self.last_state = match self.reshape_state_machine(ReshapeState::Idle)? {
    ...
    pub fn check_state(&mut self) -> FroyoResult<()> {
        if let FroyoState::Initializing = self.last_state {
            let cur_state = self.status()?;

            if let FroyoState::Good(_) = cur_state {
                self.initial_resync_complete()?;
            }
        ...
        let r_state = match self.last_state {
            FroyoState::Good(FroyoRunningState::Reshaping(ref state)) => Some(state.clone()),
            _ => return self.handle_thinpool_usage(),
        };

        if let Some(state) = r_state {
            self.last_state = match self.reshape_state_machine(state)? {
                ReshapeState::Off => FroyoState::Good(FroyoRunningState::Good),
                x => FroyoState::Good(FroyoRunningState::Reshaping(x)),
            };
        };
    ...
    pub fn is_reshapable(&self) -> bool {
        ...
        let reshaped_tot_size = self
            ...
            .map(|(_, rd)| {
                ...
                let members_present = rd.members.iter().filter_map(|rm| rm.present()).count();
    ...
    pub fn needs_reshape(&self) -> FroyoResult<bool> {
        ...
        for rd in self.raid_devs.raids.values() {
            let (r_status, _) = rd.borrow().status()?;
            match r_status {
    ...
    fn start_copy_to_safe_raid(
        &self,
        src: &Rc<RefCell<RaidLinearDev>>,
    ) -> FroyoResult<ReshapeState> {
        // Find first segment on a degraded raid, or exit
        let b_src = src.borrow();
        let (idx, raid_seg) = match b_src.segments.iter().enumerate().find(|&(_, rs)| {
        ...
        b_src.dev.table_load(&dm, &table)?;
        b_src.dev.unsuspend(&dm)?;
    ...
    fn start_copy_to_scratch(&self, src: &Rc<RefCell<RaidLinearDev>>) -> FroyoResult<ReshapeState> {
        let b_src = src.borrow();
        ...
        let raid_idxs = raid_segs.iter().map(|&(idx, _)| idx).collect::<Vec<_>>();
        let raid_segs_ls = raid_segs
            .iter()
            .map(|&(_, rs)| {
                (
                    TempLayer::Raid(rs.parent.raid()),
    ...
    fn start_copy_from_scratch(
        &mut self,
        dest: Rc<RefCell<RaidLinearDev>>,
    ) -> FroyoResult<ReshapeState> {
        if !dest
            .borrow()
            .segments
            .iter()
            .any(|seg| seg.parent.on_temp())
        {
            return Ok(ReshapeState::Idle);
        }
        ...
        let raid_segs = self.raid_devs.alloc_raid_segments(len).ok_or_else(|| {
            io::Error::new(
                io::ErrorKind::InvalidInput,
        ...
        b_dest.dev.table_load(&dm, &table)?;
        b_dest.dev.unsuspend(&dm)?;
    ...
    pub fn handle_thinpool_usage(&mut self) -> FroyoResult<()> {
        ...
        match self.thin_pool_dev.status()? {
                ...
                match status {
                    ThinPoolWorkingStatus::Good => {
                        ...
                        if remaining_data < self.thin_pool_dev.low_water_blocks {
                            self.extend_thinpool_data_dev(TPOOL_EXTEND_SECTORS)?;
                            self.save_state()?;
                        }
                        ...
                        if remaining_meta < *self.thin_pool_dev.low_water_blocks {
                            ...
                            self.extend_thinpool_meta_dev(meta_secs)?;
                            self.save_state()?;
                        }
                    }
                    _ => panic!("bad thin check_status: {:?}", status),
                }
            }
        };
    ...
    pub fn dump_status(&self) -> FroyoResult<()> {
        ...
        let tp_status = self.thin_pool_dev.status()?;
